...
pub static THIS_PROCESS: LazyLock<PathBuf> = LazyLock::new(|| current_exe().unwrap());
pub static BASE_DIR: LazyLock<PathBuf> = LazyLock::new(|| THIS_PROCESS.parent().unwrap().parent().unwrap().parent().unwrap().to_path_buf());
